...
public class NewIfcGameObject : MonoBehaviour
{
    ...
    {
        GameObject ifcGameObject = GameObject.CreatePrimitive(PrimitiveType.Cube); //큐브타입 생성
        ifcGameObject.name = elementName; //이름변경

        //Element Attributes
        IfcAttributes ifcAttributes = ifcGameObject.AddComponent<IfcAttributes>() as IfcAttributes; //생성한 오브젝트에 IfcAttributes스크립트 참조 후 ifcAttributes변수에 저장
